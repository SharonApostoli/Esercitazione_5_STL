...
int main()
{
    ...
    map<string, int> amap = {pair<string, int>("hi", 1), {"ciao", 2}};

    for(auto c: amap)
        cout << c.first << " " << c.second << endl;

    for(auto it = amap.begin(); it != amap.end(); it++) // < non valido per map iterator
        cout << it->first << " " << it->second << endl; // arrow operator = deferenza + access member

    // random access based on key
    cout << amap["hi"] << endl;
    cout << amap.at("hi") << endl; // safer tan subscript operator []: try to make a mistake

    auto itamap = amap.find("salut");
    if(itamap != amap.end())
        cout << itamap->second << endl;
    else
        cerr << "key does not exist" << endl;

    auto ret = amap.insert({"hi", 7});
    cout << boolalpha << (ret.first)->second << " " << ret.second << endl; // già presente -> second is false, non inserisco niente

    auto ret1 = amap.insert({"pippo", 7});
    cout << (ret1.first)->second << " " << ret1.second << endl; // non ancora presente -> second is true
